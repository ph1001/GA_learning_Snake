...
class Individual():
    ...
    def __init__(self,
                ...
                fitness_function = lambda x,y,z: y*100 + z ,
                weights = None,
                moves_till_stuck = 50,
        ...
        self.model.add(layers.Dense(26, activation = 'relu', input_dim = input_dim))
    ...
    def play(self, show = False):
        ...
        if self.show or show:
            score, age, move_score_sum = controlled_run(self, self.ind_number, self.evolution_step, self.games_to_play, self.verbose, self.moves_till_stuck)
        else:
            score, age, move_score_sum = controlled_run_nodisplay(self, self.ind_number, self.evolution_step, self.games_to_play, self.verbose, self.moves_till_stuck)
        ...
        self.fitness = self.fitness_function(age, score, move_score_sum)
    ...
    def control(self, game_state):
        ...
        # Calculate a score that represents how good this current game state is. Being close to food is good.
        # Using f(d) = 1 / (d + 0.1), with d being the Manhattan distance.
        #move_score = (1 / (abs(distance_food_y + distance_food_x) + 0.1))
        if distance_food_y == 0 or distance_food_x == 0:
            move_score = 10 * (1 / (abs(distance_food_y + distance_food_x) + 0.1))
        else:
            move_score = (1 / (abs(distance_food_y + distance_food_x) + 0.1))
        ...
        for gen in tqdm(range(gens), desc = 'Evolving Population'): #argument of evolve attribute
                ...
                while len(new_pop) < self.size:
                    ...
                    new_pop.append(Individual(ind_number = len(new_pop),
                                              ...
                                              moves_till_stuck = moves_till_stuck,
