    ...
    def gameLoop(score_list, age_list, verbose, moves_till_stuck):
        ...
        while not game_over:
            ...
            if snake_List == []:
                ...
                if not automatic_mode:
                    game_action = 'w'
                if automatic_mode:#first move up automatically
            ...
            # From the second move onwards      
                ...
                game_state = {'snake_Head':snake_Head, 'foodx':foodx, 'foody':foody, 'direction':(x1_old, y1_old), 'snake_List':snake_List}
                # Pass it to control()
            ...
            # In manual mode: Process the action received from the user in the same way a keyboard input would have been processed in the original version of the game
                ...
                if automatic_mode:
                    if game_action == 0: #continue same direction
                    #keep going in the same direction
                        x1_change, y1_change = x1_old, y1_old
                
                    elif game_action == 1: #turn right
                    
                        if y1_old == - snake_block: #recognize if it's moving up vertically
                            x1_change =   snake_block
                            y1_change = 0
                        elif y1_old == snake_block: #recognize if it's moving down vertically
                            x1_change =  - snake_block
                            y1_change = 0
                        elif x1_old == - snake_block: #recognize if it's moving left horizontally
                            x1_change =  0
                            y1_change = - snake_block
            ...
            age += 1
            moves += 1
            x1_old, y1_old = x1_change, y1_change
